...
import type { CreateTaskParams, ListTasksParams, SearchTasksParams } from '../types';
...
export function validatePrompt(value: unknown): string {
  ...
  if (value.length > 2000) {
    throw new ValidationError('Prompt must be less than 2000 characters', 'prompt');
  }
  return value.trim();
}
...
export function parseRepositories(value: unknown): string[] | undefined {
  ...
  if (!isString(value)) {
    throw new ValidationError('Repositories must be a comma-separated string', 'repositories');
  }
  ...
  // Validate each repo URL
  for (const repo of repos) {
    if (!repo.startsWith('http://') && !repo.startsWith('https://')) {
      throw new ValidationError(`Invalid repository URL: ${repo}`, 'repositories');
    }
  }
...
// Composite validators for command parameters
export function validateCreateTaskParams(options: Record<string, unknown>): CreateTaskParams {
  return {
...
export function validateListTasksParams(options: Record<string, unknown>): ListTasksParams {
  return {
...
export function validateSearchTasksParams(options: Record<string, unknown>): SearchTasksParams {
  return {
